...
void telaInicial(ssd1306_t *ssd, bool cor)
{
  ssd1306_fill(ssd, !cor);                         // Limpa o display
  ssd1306_rect(ssd, 3, 3, 122, 58, cor, !cor);     // Desenha um retângulo
  ssd1306_draw_string(ssd, "_^_^_^_^_", 25, 10);   // Desenha uma string
  ssd1306_draw_string(ssd, "Digite um", 25, 25);   // Desenha uma string
  ssd1306_draw_string(ssd, "caractere", 25, 35);   // Desenha uma string
  ssd1306_draw_string(ssd, "_^_^_^_^_^_", 18, 48); // Desenha uma string
  ssd1306_send_data(ssd);                          // Atualiza o display
}
void ligouVerde(ssd1306_t *ssd, bool cor)
{
  ssd1306_fill(ssd, false);                         // Limpa o display
  ssd1306_rect(ssd, 3, 3, 122, 58, cor, !cor);      // Desenha um retângulo
  ssd1306_draw_string(ssd, "_[[[\\][[[_", 25, 10);  // Desenha uma string
  ssd1306_draw_string(ssd, "Ligou o LED", 25, 25);    // Desenha uma string
  ssd1306_draw_string(ssd, " Verde", 35, 35);      // Desenha uma string
  ssd1306_draw_string(ssd, "][[[[^[[[[\\", 21, 48); // Desenha uma string
  ssd1306_send_data(ssd);                           // Atualiza o display
}
void desligouVerde(ssd1306_t *ssd, bool cor)
{
  ssd1306_fill(ssd, false);                         // Limpa o display
  ssd1306_rect(ssd, 3, 3, 122, 58, cor, !cor);      // Desenha um retângulo
  ssd1306_draw_string(ssd, "_[[[\\][[[_", 25, 10);  // Desenha uma string
  ssd1306_draw_string(ssd, "Desligou o", 25, 25);    // Desenha uma string
  ssd1306_draw_string(ssd, "LED Verde", 25, 35);      // Desenha uma string
  ssd1306_draw_string(ssd, "][[[[^[[[[\\", 21, 48); // Desenha uma string
  ssd1306_send_data(ssd);                           // Atualiza o display
}
void ligouAzul(ssd1306_t *ssd, bool cor)
{
  ssd1306_fill(ssd, false);                         // Limpa o display
  ssd1306_rect(ssd, 3, 3, 122, 58, cor, !cor);      // Desenha um retângulo
  ssd1306_draw_string(ssd, "][[[[^[[[[\\", 21, 10); // Desenha uma string
  ssd1306_draw_string(ssd, "Ligou o LED", 25, 25);    // Desenha uma string
  ssd1306_draw_string(ssd, " Azul", 35, 35);      // Desenha uma string
  ssd1306_draw_string(ssd, "_[[[\\][[[_", 25, 48);  // Desenha uma string
  ssd1306_send_data(ssd);                           // Atualiza o display
}

void desligouAzul(ssd1306_t *ssd, bool cor)
{
  ssd1306_fill(ssd, false);                         // Limpa o display
  ssd1306_rect(ssd, 3, 3, 122, 58, cor, !cor);      // Desenha um retângulo
  ssd1306_draw_string(ssd, "_[[[\\][[[_", 25, 10);  // Desenha uma string
  ssd1306_draw_string(ssd, "Desligou o", 25, 25);    // Desenha uma string
  ssd1306_draw_string(ssd, "LED Azul", 25, 35);      // Desenha uma string
  ssd1306_draw_string(ssd, "][[[[^[[[[\\", 21, 48); // Desenha uma string
  ssd1306_send_data(ssd);                           // Atualiza o display
}

void telaChar(ssd1306_t *ssd, char c)
{
  ssd1306_fill(ssd, false);                         // Limpa o display
  ssd1306_rect(ssd, 3, 3, 122, 58, false, true);    // Desenha um retângulo
  ssd1306_draw_string(ssd, "][[[[^[[[[\\", 21, 10); // Desenha uma string
  ssd1306_draw_string(ssd, "Digitou:", 25, 25);     // Desenha uma string
  ssd1306_draw_char(ssd, c, 59, 35);                // Desenha uma string
  ssd1306_draw_string(ssd, "_[[[\\][[[_", 25, 48);  // Desenha uma string
  ssd1306_send_data(ssd);
}
